...
export default function DrillFilter({ drills }: Props) {
  ...
  return (
    <div class="grid md:grid-cols-[16rem_1fr] gap-6">
      {/* Filter sidebar */}
      <aside class="bg-slate-50 border border-slate-200 rounded-lg p-4 text-sm md:sticky md:top-20 md:self-start no-print">
        <div class="flex justify-between items-center mb-3">
          <h2 class="tracking-tight font-semibold">Filters</h2>
          {(skills.length || phases.length || levels.length || maxDuration || search || sort !== 'name') ? (
            <button onClick={reset} class="text-xs text-blue-700 hover:underline">Reset</button>
          ) : null}
        </div>
        ...
        <div class="mb-4">
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Primary skill</label>
          <div class="flex flex-wrap gap-1">
            {SKILLS.map(s => (
              <button
                onClick={() => toggle(skills, s, setSkills)}
                class={`px-2 py-1 text-xs rounded border ${skills.includes(s) ? 'bg-blue-600 text-white border-blue-600' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-100'}`}
              >{s}</button>
            ))}
          </div>
        </div>
        ...
        <div class="mb-4">
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Phase</label>
          <div class="flex flex-wrap gap-1">
            {PHASES.map(p => (
              <button
                onClick={() => toggle(phases, p, setPhases)}
                class={`px-2 py-1 text-xs rounded border ${phases.includes(p) ? 'bg-blue-600 text-white border-blue-600' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-100'}`}
              >{p}</button>
            ))}
          </div>
        </div>
        ...
        <div class="mb-4">
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Age level</label>
          <div class="flex flex-wrap gap-1">
            {LEVELS.map(l => (
              <button
                onClick={() => toggle(levels, l, setLevels)}
                class={`px-2 py-1 text-xs rounded border ${levels.includes(l) ? 'bg-blue-600 text-white border-blue-600' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-100'}`}
              >{l}</button>
            ))}
          </div>
        </div>
        ...
        <div class="mb-4">
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">
            Max duration {maxDuration > 0 && <span class="text-slate-700">({maxDuration} min)</span>}
          </label>
          <input
            type="range"
            min="0"
            ...
            class="w-full"
          />
          <div class="flex justify-between text-xs text-slate-500"><span>any</span><span>60min</span></div>
        </div>

        <div>
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Sort</label>
      ...
      <div>
        ...
        <div class="overflow-x-auto rounded border border-slate-200">
          <table class="min-w-full text-sm">
            ...
            <tbody>
              {filtered.map(d => (
                <tr class="border-t border-slate-200 hover:bg-blue-50">
                  <td class="px-3 py-2"><a href={d.href} class="text-blue-700 font-medium no-underline hover:underline">{d.name}</a></td>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={6} class="px-3 py-6 text-center text-slate-500">No drills match. <button onClick={reset} class="text-blue-700 hover:underline">Reset filters</button></td></tr>
              )}
            </tbody>
          </table>
        </div>
