...
complexity: high
when-to-use: "Modern technique overlay for blockers closing long distances on pin sets; the current default at HS+ women's volleyball for lateral coverage."
alternatives: [read-blocking, commit-blocking]
sources: [aoc-2024-slabe-blocking, aoc-2023-swing-blocking-when, aoc-2024-blocking-fundamentals, aoc-2016-liskevych-blocking-what-matters, aoc-2015-mccutcheon-blocking-keys, aoc-2017-liskevych-asics-blocking]
---
...
- **Athletic blockers:** the run-with-arms movement demands coordination between feet, arm swing, and jump load. Blockers who can't integrate the arm swing smoothly lose both the travel advantage and the jump benefit.
- **Middles with closing discipline:** the highest-value swing-blocking rep is the middle closing to a pin attack; the middle must swing, gather, and square to the net before jumping rather than swinging into a rotated shoulder position that splits the hands.
- **Blockers who can land balanced:** swing blocking generates more momentum into the jump, which means more momentum to absorb on landing. Balanced landings are a core block fundamental and the cost of bad landings scales with swing velocity [aoc-2024-blocking-fundamentals].
- **Trained footwork layering:** the swing is layered on top of shuffle, crossover, and a base stance; teams install swing after the underlying footwork is stable.
...
## Weaknesses
- **Learning curve.** Swing blocking takes more practice time than static or shuffle footwork. Murczek's critique centers on this opportunity cost: time spent on swing might be better spent on attacking or passing for many teams [aoc-2023-swing-blocking-when].
- **Penetration claims are contested.** Murczek cites video evidence of players demonstrating both techniques without improving net penetration with the swing; the advertised benefit may not show up in practice for all blockers [aoc-2023-swing-blocking-when].
- **Landing control.** Swing blocking generates more momentum, which means more force to absorb on landing. Poor landings cost blockers a closed rotation and can increase injury risk if taught without landing discipline [aoc-2024-blocking-fundamentals].
- **Hands-first teaching can erode.** Liskevych's concern is that swing focus drifts attention from the hands and stance to the feet and arms. A swing blocker who loses hands discipline is a blocker who arrives high and wrong [aoc-2016-liskevych-blocking-what-matters].
