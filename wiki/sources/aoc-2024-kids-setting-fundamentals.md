---
type: source
source-type: article
title: "Volleyball training for kids: Teaching the fundamentals of setting"
author: "Ruth Nelson"
year: 2024
citation-key: aoc-2024-kids-setting-fundamentals
raw-file: raw/articles/aoc-2024-kids-setting-fundamentals.md
url: https://www.theartofcoachingvolleyball.com/volleyball-training-for-kids-teaching-the-fundamentals-of-setting/
topics: [youth, fundamentals, setting]
featured-coaches: [ruth-nelson]
schools: [art-of-coaching-volleyball]
trust-tier: 2
---

# Volleyball training for kids: Teaching the fundamentals of setting

## Summary
Ruth Nelson lays out a beginner-friendly framework for teaching setting to children as young as 5, starting with catch-and-set progressions and emphasizing that good body mechanics are hard to retrain once a player is older and stronger.

## Key claims / ideas
- Setting can be introduced as early as age 5 using catch-and-set drills before moving to live setting.
- Hand shape: triangle above the forehead, thumbs not touching; elbows slightly forward; feet shoulder-width with right foot forward; knees bent.
- Power comes from leg extension plus follow-through rather than the hands alone.
- Young players who can't set 5-10 feet above the net should stay with underhand passing first.
- Bad mechanics ingrained early are very hard to change later, so coaches must prioritize form over volume.

## Topics covered
- Catch-and-set progression for beginners
- Footwork patterns (left-to-right shuffle, short/deep shuffle, cross-court)
- Parent-involved skill analysis (BYOP)
- Criteria for when to transition from passing to setting a ball

## Where it's cited
*(populated as wiki pages cite this source)*

## Access
- Raw file: [[aoc-2024-kids-setting-fundamentals]]
- URL: https://www.theartofcoachingvolleyball.com/volleyball-training-for-kids-teaching-the-fundamentals-of-setting/
- Trust tier: 2: AOC
- Ingested via: aoc-mcp on 2026-04-23
