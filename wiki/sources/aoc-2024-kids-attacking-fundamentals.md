---
type: source
source-type: article
title: "Volleyball training for kids: Teaching the fundamentals of attacking"
author: "Ruth Nelson"
year: 2024
citation-key: aoc-2024-kids-attacking-fundamentals
raw-file: raw/articles/aoc-2024-kids-attacking-fundamentals.md
url: https://www.theartofcoachingvolleyball.com/volleyball-training-for-kids-teaching-the-fundamentals-of-attacking/
topics: [youth, fundamentals, attacking, spiking]
featured-coaches: [ruth-nelson]
schools: [art-of-coaching-volleyball]
trust-tier: 2
---

# Volleyball training for kids: Teaching the fundamentals of attacking

## Summary
Ruth Nelson presents a three-stage progression for teaching attacking to kids: down-ball (no jump), 2-step modified approach, and full 3-step approach, with age-specific focus from 4-6 through 8-11.

## Key claims / ideas
- Stage 1 "puppet position" down-ball work builds arm-swing muscle memory before any jumping.
- Stage 2 introduces the 2-step modified approach with isolated jumping and accurate tosses slightly over the hitting arm.
- Stage 3 is the full 3-step approach emphasizing explosive takeoff and contact at the highest point.
- Age windows: 4-6 focus on down-ball; 7-8 introduce 2-step; 8-11 progress toward full 3-step.
- Good attacks depend on good passing and setting upstream, can't isolate hitting from ball-control quality.

## Topics covered
- Down-ball arm-swing mechanics
- Approach footwork progression (2-step → 3-step)
- Age-appropriate attacking progressions
- Toss quality and parent/coach feeding

## Where it's cited
*(populated as wiki pages cite this source)*

## Access
- Raw file: [[aoc-2024-kids-attacking-fundamentals]]
- URL: https://www.theartofcoachingvolleyball.com/volleyball-training-for-kids-teaching-the-fundamentals-of-attacking/
- Trust tier: 2: AOC
- Ingested via: aoc-mcp on 2026-04-23
