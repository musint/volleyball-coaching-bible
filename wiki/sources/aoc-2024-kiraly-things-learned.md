---
type: source
source-type: article
title: "Karch Kiraly: Things I learned at the top level of volleyball"
author: "Karch Kiraly"
year: 2024
citation-key: aoc-2024-kiraly-things-learned
raw-file: raw/articles/aoc-2024-kiraly-things-learned.md
url: https://www.theartofcoachingvolleyball.com/karch-kiraly-things-i-learned-at-the-top-level-of-volleyball/
topics: [coaching-philosophy, culture, leadership, feedback, managing-a-program]
featured-coaches: [karch-kiraly, luka-slabe]
schools: [art-of-coaching-volleyball, usa-volleyball-cap]
trust-tier: 2
---

# Karch Kiraly: Things I learned at the top level of volleyball

## Summary
Video-backed, with a richer text body (~440 words). Kiraly and Luka Slabe share lessons from coaching at the elite level: boundaries with athletes, modeling standards, not assuming prior knowledge, holding the line on what you tolerate, and building programs that make motivation unnecessary because learning is the reward.

## Key claims / ideas
- Slabe: coaches and players shouldn't be "friends" until after retirement; the job is athlete development, not keeping players happy.
- Standards are set by coach behavior, "preaching one thing and doing another doesn't work."
- Don't assume competence: Karch discovered national-team athletes couldn't identify rotations on video, then fixed it with video + quizzing.
- Psychological safety ("it's OK not to know") is a precondition for honest answers and real learning.
- "Coaches get what they tolerate", untolerated lapses (e.g., not pursuing a ball) otherwise become the norm.
- Players are happiest when they are learning; feedback is the currency they ask for. A learning-centered program reduces the need to motivate.

## Topics covered
- Coach-athlete relational boundaries
- Modeling, standards, and accountability ("what you tolerate")
- Diagnosing hidden knowledge gaps via video review
- Motivation via learning environment design
- Program management at the elite level

## Where it's cited
*(populated as wiki pages cite this source)*

## Access
- Raw file: [[aoc-2024-kiraly-things-learned]]
- URL: https://www.theartofcoachingvolleyball.com/karch-kiraly-things-i-learned-at-the-top-level-of-volleyball/
- Trust tier: 2: AOC, Karch Kiraly named author
- Ingested via: aoc-mcp on 2026-04-23
