---
type: source
source-type: article
title: "Setter training for volleyball: Disguising the back set"
author: "John Dunning"
year: 2023
citation-key: aoc-2023-dunning-disguising-back-set
raw-file: raw/articles/aoc-2023-dunning-disguising-back-set.md
url: https://www.theartofcoachingvolleyball.com/setter-training-for-volleyball-disguising-the-back-set/
topics: [setter, setting, back-set, disguise, deception]
featured-coaches: [john-dunning]
schools: [art-of-coaching-volleyball]
trust-tier: 2
---

# Setter training for volleyball: Disguising the back set

## Summary
Former Stanford head coach John Dunning teaches the disguise mechanics of the back set: eliminate the body-position giveaways, forward hip thrust, arched back, looking behind, and learn to "turn the hands over" so the setter's posture stays neutral long enough to freeze opposing blockers.

## Key claims / ideas
- Thrusting hips forward, arching the back, and looking behind are "dead giveaways" to opposing blockers and defenders.
- Neutral body position + "turning the hands over" is the mechanical key to disguising the back set.
- The back set has become both more difficult and more important in the modern game.

## Topics covered
- Back-set disguise and deception
- Setter body-position tells to eliminate
- Hand-turnover mechanics

## Where it's cited
*(populated as wiki pages cite this source)*

## Access
- Raw file: [[aoc-2023-dunning-disguising-back-set]]
- URL: https://www.theartofcoachingvolleyball.com/setter-training-for-volleyball-disguising-the-back-set/
- Trust tier: 2: AOC
- Ingested via: aoc-mcp on 2026-04-23
