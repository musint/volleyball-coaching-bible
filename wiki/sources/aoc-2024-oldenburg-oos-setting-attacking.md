---
type: source
source-type: article
title: "Out-of-system setting and attacking drill"
author: "Jen Flynn Oldenburg"
year: 2024
citation-key: aoc-2024-oldenburg-oos-setting-attacking
raw-file: raw/articles/aoc-2024-oldenburg-oos-setting-attacking.md
url: https://www.theartofcoachingvolleyball.com/out-of-system-setting-and-attacking-drill/
topics: [out-of-system, setter, setting, well-rounded-players, digging]
featured-coaches: [jen-flynn-oldenburg]
schools: [art-of-coaching-volleyball]
trust-tier: 2
---

# Out-of-system setting and attacking drill

## Summary
Jen Flynn Oldenburg extends her OOS dig-and-set drill by adding attacking: coach hits, digger digs to non-setter, non-setter sets OOS, pin hitter attacks. Everyone rotates through all roles, liberos included, to build cross-skill capability and empathy for hitters. Played to 25 in a row, tape = zero.

## Key claims / ideas
- "Everybody should be able to dig a ball. Everybody should be able to set an out-of-system ball. Everybody should be able to hit a ball hard to a corner."
- Liberos should take swings in this drill.
- Cross-training the roles gives pin hitters an appreciation for set height and location demands.
- Game-to-25 with tape = reset enforces accuracy under team pressure.

## Topics covered
- Out-of-system setting and attacking
- Cross-role training / positionless skill development
- Libero swinging
- Drill scoring for team accountability

## Where it's cited
*(populated as wiki pages cite this source)*

## Access
- Raw file: [[aoc-2024-oldenburg-oos-setting-attacking]]
- URL: https://www.theartofcoachingvolleyball.com/out-of-system-setting-and-attacking-drill/
- Trust tier: 2: AOC
- Ingested via: aoc-mcp on 2026-04-23
