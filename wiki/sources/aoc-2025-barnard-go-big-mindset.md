---
type: source
source-type: article
title: "Why outside hitters need a 'go big' mindset"
author: "Mark Barnard"
year: 2025
citation-key: aoc-2025-barnard-go-big-mindset
raw-file: raw/articles/aoc-2025-barnard-go-big-mindset.md
url: https://www.theartofcoachingvolleyball.com/why-outside-hitters-need-a-go-big-mindset/
topics: [outside-hitter, hitting, mental, resilience, arm-swing, approach-timing]
featured-coaches: [mark-barnard]
schools: [art-of-coaching-volleyball]
trust-tier: 2
---

# Why outside hitters need a 'go big' mindset

## Summary
Mark Barnard (Oregon State) describes the outside hitter's mental stance as "I'm going to save this team" and explicitly prohibits tipping the ball immediately after being blocked.

## Key claims / ideas
- OH mindset cue: "I'm going to get this point. I'm going to save this team."
- Job difficulty framed: outsides get bailout sets out-of-system and face full blocks repeatedly.
- Anti-pattern after a block: do NOT tip the next ball, "swing again, swing harder, swing higher."
- Resilience is a trainable skill built through repeated reset after getting blocked.
- Timing rule: whatever your start trigger, once you begin the approach, don't stop until contact, higher sets = longer wait before starting.

## Topics covered
- OH mental resilience
- Post-block response (anti-tip rule)
- Approach-timing non-stop principle
- Adaptation to set height via start timing

## Where it's cited
*(populated as wiki pages cite this source)*

## Access
- Raw file: [[aoc-2025-barnard-go-big-mindset]]
- URL: https://www.theartofcoachingvolleyball.com/why-outside-hitters-need-a-go-big-mindset/
- Trust tier: 2: AOC
- Ingested via: aoc-mcp on 2026-04-23
