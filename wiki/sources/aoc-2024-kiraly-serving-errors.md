---
type: source
source-type: article
title: "Karch Kiraly’s 2-minute take: Serving effectively means making a few errors"
author: "Karch Kiraly"
year: 2024
citation-key: aoc-2024-kiraly-serving-errors
raw-file: raw/articles/aoc-2024-kiraly-serving-errors.md
url: https://www.theartofcoachingvolleyball.com/karch-kiralys-2-minute-take-serving-effectively-means-making-a-few-errors/
topics: [serving, game-strategy, risk-reward, stats, pressure]
featured-coaches: [karch-kiraly]
schools: [art-of-coaching-volleyball, usa-volleyball-cap]
trust-tier: 2
---

# Karch Kiraly’s 2-minute take: Serving effectively means making a few errors

## Summary
Video-backed, minimal text body. Kiraly argues the risk-reward math of the serve: at most levels, a team getting every serve in isn't applying enough pressure. Going for harder, flatter serves and accepting some misses is the expected-value play.

## Key claims / ideas
- If your team lands every serve, you aren't pressuring the opponent enough.
- Serving is "a huge point-scoring weapon", treat it as offense rather than ball-start.
- The right target is harder, flatter serves with an accepted error rate.
- The framing is EV: some misses are the price of serves that actually disrupt opponent reception.

## Topics covered
- Serving strategy and tough-serve philosophy
- Risk-reward tradeoff on the serve
- Serving as a scoring weapon
- Pressure-serving decisions

## Where it's cited
*(populated as wiki pages cite this source)*

## Access
- Raw file: [[aoc-2024-kiraly-serving-errors]]
- URL: https://www.theartofcoachingvolleyball.com/karch-kiralys-2-minute-take-serving-effectively-means-making-a-few-errors/
- Trust tier: 2: AOC, Karch Kiraly named author
- Ingested via: aoc-mcp on 2026-04-23
