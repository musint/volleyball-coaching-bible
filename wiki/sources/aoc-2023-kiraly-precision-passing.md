---
type: source
source-type: article
title: "Precision passing with Karch Kiraly"
author: "Karch Kiraly"
year: 2023
citation-key: aoc-2023-kiraly-precision-passing
raw-file: raw/articles/aoc-2023-kiraly-precision-passing.md
url: https://www.theartofcoachingvolleyball.com/passing-with-karch-kiraly/
topics: [passing, drills, game-like-training, coaching-philosophy]
featured-coaches: [karch-kiraly]
schools: [art-of-coaching-volleyball, usa-volleyball-cap]
trust-tier: 2
---

# Precision passing with Karch Kiraly

## Summary
Kiraly frames passing training around game-like reps, chaos and randomness, not spoon-fed drills, on the premise that game-like training drives skill retention. The page introduces a game-like passing drill.

## Key claims / ideas
- "Scientifically speaking, game-like training leads to skill retention."
- Players should be trained to get good at competing, not at training.
- Rep design should use chaos/randomness, not controlled feeds.
- Passing belongs inside game-like situational drills.

## Topics covered
- Passing technique drill design
- Game-like (representative) training
- Coaching philosophy on rep design
- Competitive training over controlled reps

## Where it's cited
*(populated as wiki pages cite this source)*

## Access
- Raw file: [[aoc-2023-kiraly-precision-passing]]
- URL: https://www.theartofcoachingvolleyball.com/passing-with-karch-kiraly/
- Trust tier: 2: AOC, Kiraly named
- Ingested via: aoc-mcp on 2026-04-23
