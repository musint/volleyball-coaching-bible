---
type: source
source-type: article
title: "'Dig to the X' progression drill with Terry Liskevych"
author: "Terry Liskevych"
year: 2015
citation-key: aoc-2015-liskevych-dig-to-x-progression
raw-file: raw/articles/aoc-2015-liskevych-dig-to-x-progression.md
url: https://www.theartofcoachingvolleyball.com/dig-to-the-x-progression-drill-with-terry-liskevych/
topics: [drills, out-of-system, transition, defense, progression]
featured-coaches: [terry-liskevych]
schools: [art-of-coaching-volleyball]
trust-tier: 2
---

# 'Dig to the X' progression drill with Terry Liskevych

## Summary
Terry Liskevych's (Oregon State) three-stage dig-set progression. The X is marked just behind the 3-meter line midcourt, the target for every out-of-system dig. Stage 1: dig-set to a static LF target. Stage 2: LF becomes an attacker and must approach after the dig. Stage 3: cooperative over-the-net version with attackers on both sides.

## Key claims / ideas
- Fixing a target (the X) for out-of-system digs institutionalizes a high-percentage set location.
- The 3-meter-line midcourt X gives the setter a large vertical window to convert into a set.
- Progression adds one complexity at a time: static target → live attacker → over-the-net cooperative rally.
- Trains the whole out-of-system chain: dig location → set → transition attack.

## Topics covered
- Out-of-system dig targeting
- Dig-set-attack progression
- Cooperative over-the-net transition

## Where it's cited
*(populated as wiki pages cite this source)*

## Access
- Raw file: [[aoc-2015-liskevych-dig-to-x-progression]]
- URL: https://www.theartofcoachingvolleyball.com/dig-to-the-x-progression-drill-with-terry-liskevych/
- Trust tier: 2: AOC
- Ingested via: aoc-mcp on 2026-04-23
