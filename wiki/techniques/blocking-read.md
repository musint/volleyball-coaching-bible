...
related-drills: [block-touch-drill, commit-block-trigger, read-blocking-progression]
related-techniques: [blocking-footwork, blocking-hand-position, blocking-swing]
schools-perspectives:
  art-of-coaching-volleyball: "Reading is primary; Kiraly's B-S-B-H eye-progression (ball → setter → ball → hitter) is the teaching spine."
  gold-medal-squared: "Motor-learning frame: train reading via random/variable practice so pattern recognition transfers to live play; eyework is an explicit skill (setter first, hitter by jump time)."
  ecological-dynamics: "Perception-action coupling: blockers pick up affordances directly from the setter/hitter system in situ; reading is trained by preserving representative task constraints, not by drilling eye-targets in isolation."
sources:
...
1. **Teach the sequence before the speed.** The dominant modern pattern is
...
7. **Teach common tendencies.** On bad passes, most teams go left. Most
   setters tip when scrambling. Most hitters hit their strongest shot on
   2-0. Give blockers these patterns explicitly; experienced coaches call
   this "game IQ" and it is teachable, not innate.
8. **Verbalize the read.** A reliable coaching method: stop play, ask
   "what did you see?" Then give the blocker what you saw. Over reps the
   athlete's internal verbalization becomes the read.

**Bunch-read layer.**

9. **Introduce bunch-read awareness at HS+.** When pin blockers start
...
## Common errors
- **Staring at the ball.** The most common failure at every level. The ball
  doesn't tell the blocker anything about the attack until the hit; the
  setter and hitter do. Fix: drill the eye-progression away from live play
  until it's automatic.
- **Guessing.** Blocker decides before the set leaves the setter's hands
...
- **No read bank.** Blocker treats every rally as a new puzzle because
  ...
  shoot 1s on perfect passes, etc.). Fix: teach tendencies explicitly
  during film and practice.

## Schools-perspectives
All three schools below agree that blocking is first a reading problem and
only second a movement problem; they disagree on how reading is best
trained.

**Art of Coaching Volleyball** is the most vocal modern advocate for reading
as a distinct teachable skill. Kiraly's framing is that reading is a part
...
tempo) doesn't transfer when the real information sources differ.
Practically, this converges with GMS on the centrality of random/variable
practice against real attackers; philosophically it rejects the
information-processing "see-know-decide" sequence AOC uses as its
teaching spine.

**Practical takeaway.** All three schools will build a competent reader if
