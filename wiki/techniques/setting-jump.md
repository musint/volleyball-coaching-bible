...
subskill: jump
positions: [setter]
related-drills: [jump-setting-progression, three-setter-rotation]
related-techniques: [setting-hands, setting-backset, setting-out-of-system]
sources:
...
**Readiness check.** A setter should not train jump-setting before standing hand-setting is stable. Cues: consistent ball-shaped hand contact on the sides of the ball, wrist hinge without leg-snap compensation, held follow-through on the target line, ability to set pace-consistent to pins from a moving pass. If any of these is absent, regress to [[setting-hands]] before adding air.

**When to start training.** For club programs, start when the setter can consistently deliver clean overhead sets from a moving pass and has the leg strength to reach a point above the net while maintaining trunk control. For 14U setters this is usually late in the club season or into the summer after, not day one. HS varsity setters should be training jump-setting year-round. College setters are jump-setting any in-system ball to the front-row pins as a default.

**Progression.**

1. **Stationary vertical jump-set.** From a two-foot takeoff directly under a tossed ball, jump, set to a target, land under control. No horizontal movement. The drill tests whether hand mechanics survive the transition to air.
2. **Jump-set from an approach.** Setter shuffles to the target zone, then jumps and sets. Timing is now the coaching variable: plant foot down, gather, jump so the set is at the peak, not on the way up. Height must still be consistent.
...
- **Jump-setting from an out-of-system pass.** A jump set off a pass that is ten feet off the net, low, or moving laterally is a low-probability set. The setter bails to a standing set or a redirect. Correction: jump only from a clean in-system pass; beat the ball to the target area first, then decide whether to go up.
- **Poor timing with the middle.** Setter jumps too early, so the ball leaves the hands before the middle reaches peak; or too late, so the middle is descending. Correction: pair the setter and a named middle in practice, run repeated middle-timing reps until the jump-and-release is synchronized.
- **Body rotation in the air.** Hips or shoulders open early to an intended pin; the block reads it. Correction: video review to find rotation, constraint drill where setter must set opposite of the obvious pass-side direction, and reinforce neutral shoulder habit on standing sets first.
...
Middle-timing jump-set reps, jump-set-only controlled games, and second-contact attack drills. Drill pages pending Wave 4 ingest; see [[setting-hands]] and [[setting-out-of-system]] for adjacent training.
