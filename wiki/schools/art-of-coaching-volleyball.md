---
type: school
name: Art of Coaching Volleyball
origin: USA (Oregon)
founders: [terry-liskevych, john-dunning, russ-rose]
core-principles: [coach-development, video-based-learning, breadth-of-voices, reading-centric-pedagogy, fundamentals-first, coaching-as-craft]
associated-coaches: [terry-liskevych, john-dunning, russ-rose, karch-kiraly, hugh-mccutcheon, jim-stone, tod-mattox, luka-slabe, gina-schmidt, ray-bechard, ben-josephson, ruth-nelson]
related-schools: [usa-volleyball, gold-medal-squared, japanese-training, ecological-dynamics]
sources:
...
bronze) and an Oregon State base; Dunning brought the Stanford/Pacific
record (888-185 across 32 D-I seasons, five NCAA titles)
[aoc-2017-liskevych-dunning-tribute]; Rose anchored the venture with 1,309
wins and seven NCAA titles across 42 years at Penn State
...
multi-coach panels [aoc-2017-one-big-thing-panel][aoc-2020-passing-principles],
and the "10 things I've learned" essays from the founders
[aoc-2017-rose-10-things-learned][aoc-2017-dunning-10-things][liskevych-2015-aoc-10-things].
Content is tiered: a large body of fundamentals and short clips is free;
deeper clinics, full-length panels, and the Premium library sit behind a
paid membership [aoc-2017-one-big-thing-panel][aoc-2018-coaching-iy-generation].

AOC is best understood as a coach-education infrastructure layer, not a
...
single method is correct" [aoc-2024-legends-liskevych-kiraly]. The cost of
that breadth, raised below, is a catalog that can leave coaches with dozens
of compatible cues and no opinionated path through them.
...
- **Breadth of voices over methodological purity.** AOC routinely
  presents conflicting coach perspectives on the same topic without resolving
  them. Russ Rose, John Dunning, and Jim Stone share a single passing-principles
  panel despite holding meaningfully different teaching emphases
...
- **Reading-centric pedagogy.** Multiple AOC pieces argue that reading, 
  ...
  [aoc-2024-kiraly-reads-every-skill] and Josephson's "look, see, decide"
  decomposition [aoc-2025-training-reading-look-see-decide] are the
  organization's two most frequently-restated reading frameworks. Slabe's bunch
  read blocking carries the same logic to a system-level scheme
  [aoc-2026-bunch-read-blocking].

- **Fundamentals-first orientation.** AOC's "10 things I've learned" essays
...
- **Coaching as craft, learning as job one.** The catalog's editorial line
  on coach development frames coaching as a craft refined over decades, not a
  set of techniques to be downloaded. Kiraly: "the number one job of everyone
  in the program is to be a learner" [aoc-2024-kiraly-culture]. Dunning's first
  lesson: "Listen. Stop solving things. Just listen, and maybe ask questions.
  You will evolve" [aoc-2017-dunning-10-things]. Liskevych: "failure is tuition
  for the future" [liskevych-2021-volleybrains-no-shortcuts].

## Methodology

AOC does not publish a single methodology document. The de facto methodology is
inferable from the editorial pattern across its catalog and from the published
work of the three founders.

**Skill teaching: decomposition then integration.** AOC fundamentals content
...
exaggeration as a separate cue) before integrating with a pass-to-hit drill
[aoc-2024-attack-approach]. Slabe layers static, swing, and combination
blocking as complementary tools [aoc-2024-slabe-blocking]. Stone's platform
management piece is a single-concept correction (don't pre-form the
platform; let the lower body arrive first), packaged as a Quick Hit and
tied to youth development [aoc-2018-stone-platform-management]. No AOC
document states the decomposition pattern as house pedagogy, but it is
consistent enough across coaches to count as one.

**Practice design: learn-by-doing inside game-shaped tasks.** Once
...
Bonus-Point 6v6 (extra points for emphasized objectives like tooling
blockers) with guidance to simplify the system, test inherited
assumptions, and prefer honest feedback to reflexive praise
[aoc-2021-mattox-early-season-practice]. The pattern: short component
work, then game-shaped competition with scoring constraints that direct
attention.

**Reading as a cross-skill thread.** Reading is woven through every skill
block rather than isolated as its own. Dunning's 2020 clinic argues
reading is "frequently ignored" but "trainable across every skill, not
just defense" [aoc-2020-reading-every-skill]. Kiraly trains it via
questioning ("what do I see? what do I know? how do I respond?") and the
B-S-B-H (ball-setter-ball-hitter) eye sequence
[aoc-2024-kiraly-reads-every-skill]. Josephson decomposes reading into
look-see-decide, with the look ("approach angle, hips, shoulder angle,
hand") as the teachable layer [aoc-2025-training-reading-look-see-decide].
The practice-design implication, per Schmidt and Josephson: embed reading
inside live drills, ask players to verbalize what they see, and treat
pattern recognition as coachable rather than emergent
[aoc-2026-training-reading].
...
- **Posture-pursuit-platform** (Bechard) and the **B-S-B-H eye sequence**
  ...
  [aoc-2025-passing-posture-pursuit-platform][aoc-2024-kiraly-reads-every-skill].

**Culture work as first-class content.** Much of the catalog treats
program culture, communication, and player relationships as primary
coaching material, not support. Dunning's Chapel Hill 2017 chemistry
clinic frames culture as a deliberate output [aoc-2018-dunning-team-culture];
his Stanford match-prep piece names "be the most relaxed team in the gym"
as a tactical assertion [aoc-2017-dunning-stanford-character]. Rose's
program is portrayed through an "everything matters" / "no flatliners"
frame [aoc-2022-rose-coach-for-life][aoc-2019-what-learned-from-rose].
Kiraly's elite-level lessons piece argues "coaches get what they
tolerate" as a behavioral law [aoc-2024-kiraly-things-learned], and his
culture session states the bedrock value: be a learner
[aoc-2024-kiraly-culture]. Generational adaptation is also explicit content:
the 2018 "Coaching the iY Generation" piece (Dagenais, citing McCutcheon
and Mack Brown) addresses athletes raised by Generation X parents and
their distinct communication needs [aoc-2018-coaching-iy-generation].

## Notable practitioners

The AOC contributor list functions as a near-complete prosopography of
...
Laurie Eisler (Alberta), Todd Dagenais (UCF), and many others on panels and
single-piece contributions [aoc-2017-one-big-thing-panel].
...
- **vs. [[gold-medal-squared]] (GMS).** GMS, founded by Carl McGown in 1985
  ...
  Keegan Cook, Ryan McGuyre per [gms-nd-about-us]) share a common
  technical line; AOC contributors do not. The contrast is breadth versus
...
- **vs. [[ecological-dynamics]] / constraints-led approach.** AOC's
  reading-centric pedagogy is nominally aligned with ecological-dynamics
  language about perception-action coupling, but AOC typically frames
  skills as decomposable (passing = posture + pursuit + platform
  [aoc-2025-passing-posture-pursuit-platform]; attack = approach tempo +
  arm-swing + acceleration cue [aoc-2024-attack-approach]) in ways the
  ...
  *information-movement coupling* athletes use in competition, so a
  ...
  misleading because athletes develop adaptive relationships with
  environments rather than store reproducible motor templates
  [woods-2020-sport-ecology-designers]. AOC contributors have not
  engaged this critique directly in the ingested sample; alignment with
  ecological dynamics is at the level of language ("read"), not
  methodology.

## Critiques and limitations

- **Premium paywall.** A large body of fundamentals and short clips is
...
- **Breadth-over-depth risk.** The same editorial policy that gives AOC
  its range can leave a coach with twenty compatible cues for one skill
  and no guidance on which to lead with. Rose, Dunning, and Stone share
  a 2020 passing-principles panel without resolving where their emphases
  diverge [aoc-2020-passing-principles]; Bechard's posture-pursuit-platform
  [aoc-2025-passing-posture-pursuit-platform], Stone's platform-management
  Quick Hit [aoc-2018-stone-platform-management], and Liskevych's ASICS
  passing segment [aoc-2017-liskevych-asics-passing] all coexist with
  synthesis left to the user. Functional for experienced coaches with
  their own pedagogy; harder for newer coaches building one.

- **Video-first medium constrains argument depth.** Most AOC pieces center
  on a short clip. The format serves the show-don't-tell motor-learning
  intuition [aoc-2024-motor-learning] but limits capacity for the
  written, citation-heavy exposition the motor-learning and
  ecological-dynamics literatures require
  [pinder-2011-representative-learning-design][woods-2020-sport-ecology-designers].

- **U.S. women's college skew.** The founders' CVs (Penn State, Stanford,
  Oregon State, USA Women's NT) and the contributor list (Kansas, Texas,
  Simon Fraser, Bishop's School, Ohio State) skew heavily toward U.S.
  women's collegiate volleyball. Beach volleyball is comparatively
  under-covered, and men's-specific and international content is thinner.
  AOC is best-in-class U.S. coach-education, not a global product.
...
- **Opaque social distribution.** The AOC Instagram (@artofcoachingvb)
  functions as a real-time distribution layer, but posts are often gated
  behind the IG login wall when fetched programmatically
  [ig-artofcoachingvb-20260423]. Discoverability outside a paid
  membership and a logged-in IG account is patchier than the public web
  catalog suggests.
