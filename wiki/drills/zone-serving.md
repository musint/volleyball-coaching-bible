...
secondary-skills: [tactics]
techniques: [serving-float, serving-jump-float]
phase: skill
team-size-min: 6
team-size-max: 12
duration-min: 12
...
sources: [aoc-2024-rose-best-servers]
video-url:
...
## Variations
- **Opponent-specific**: scout a hypothetical opponent aloud; servers target accordingly.
- **Short/deep pairs**: restrict each server to one deep and one short zone per cycle.
- **Seam-focus**: replace number cards with tape lines on the seams between passers.

## Adaptations by level
- **14U**: three-zone version (1, 5, 6 deep); coach says the zone, no hand signals; success = ball in the target half.
- **HS**: full six-zone version with hand signals in the second half; include one seam per cycle.
- **College**: six zones plus both seams; require both float and jump-float execution in the same block.
