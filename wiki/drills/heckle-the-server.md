---
type: drill
name: Heckle the Server
primary-skill: serving
secondary-skills: [mental]
techniques: [serving-float, serving-jump-float, serving-hybrid]
phase: skill
team-size-min: 8
team-size-max: 14
duration-min: 10
...
sources: [aoc-2024-kiraly-serving-pressure]
video-url:
variations: []
---
...
2. Hecklers introduce escalating distraction while serving: starts with light crowd noise, escalates to loud fake-cheers, synchronized clapping, movement in the server's peripheral vision.
3. Server must hit a defined target zone (a tossed mat, a corner cone) to score a point; any service error loses a point.
4. Rotate through team; each server gets the same volume of attempts under the same heckling intensity.
...
- **HS:** moderate heckling with escalation; track hit rate across practice weeks.
- **College:** full match-volume crowd simulation; use recorded crowd noise if available for consistent load.
