---
type: drill
name: Reactive Jumping
primary-skill: blocking
secondary-skills: [hitting]
techniques: [blocking-footwork, hitting-approach]
phase: conditioning
team-size-min: 3
team-size-max: 12
duration-min: 10
...
sources: [aoc-2024-kids-passing-fundamentals]
video-url:
variations: [jump-warmup, conditioning-court-sprints]
---
...
2. **Cue → direction + jump.** Coach points left/right; athlete shuffles one step and jumps at the new spot. 6-8 reps per side.
3. **Cue → approach jump.** Coach cues from 10 ft off the net; athlete takes a 2- or 3-step approach and jumps. 4-6 reps.
...
## Variations
- **Block-and-transition.** Cue a landing direction after the jump so athletes train landing-to-pursuit; pairs with [[defense-pursuit]].
- **Partner mirror.** Pair blockers; one reacts to the coach, partner mirrors off the first blocker.
- **Approach-focused.** Replace block reps with reactive approaches to zone 4 or zone 2.

## Adaptations by level
- **14U:** cues only, low reactive-jump volume, heavy rest between cues. 5-6 minutes as a finisher.
- **HS:** full drill, session-total jumps capped per staff S&C guidance.
- **College:** run as one option inside a programmed plyo block owned by S&C staff.
